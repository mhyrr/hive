...
Use the council (convene_council) when you face a real fork in the road, not for validation. Use HIVE memory for context and to record what you learn. As you shift between kinds of work, call search_taste for the matching category (DESIGN before you architect, IMPLEMENTATION before you build, TEST_EVAL before you write tests) — it returns the approved judgments on how to do that work well, and hits are canon. Update tickets if the goal references them.
