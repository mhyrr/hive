...
const BETS_FILE =
  "---\nname: bets\ncadence: @nightly\nscope: runs\nwindow: 168h\nmodel: judgment\nvenue: briefing\nautonomy: act\n---\n\nWhat contrarian bets should we be thinking about?";
...
describe("watch detail page", () => {
  ...
  async function logCall(name: string, at: Date, over: Partial<Parameters<typeof writeInvocationLog>[0]> = {}) {
    const { watch } = parseWatchFile(BETS_FILE, join(paths.watchesDir, `${name}.md`), null);
    await writeInvocationLog({
      ...
      systemPrompt: "You are a HIVE watch — Watch: bets.",
      userContent: "# Watch digest: bets\nWindow: last 168h.\n\n## Nightly runs in window\n...",
      output: "Bet 1 — ship the dispute queue.",
      outcome: "surfaced",
      durationMs: 60_019,
      ...over,
    });
  ...
  test("spec, live prompt, and clamped autonomy", async () => {
    ...
    await writeFile(join(paths.watchesDir, "bets.md"), BETS_FILE);

    const data = await collectWatchDetailPage(paths, "bets");
    ...
    expect(data!.systemPrompt).toContain("Watch: bets");
    ...
    expect(html).toContain("What contrarian bets should we be thinking about?");
    expect(html).toContain("act → observe");
    expect(html).toContain("7d"); // window: 168h, shown in the largest clean unit
    expect(html).toContain("No model calls logged yet");
  });

  test("invocation history replays the exact prompts that were sent", async () => {
    await writeFile(join(paths.watchesDir, "bets.md"), BETS_FILE);
    await logCall("bets", new Date(ANCHOR.getTime() - 4 * 3_600_000));
    await logCall("bets", new Date(ANCHOR.getTime() - 28 * 3_600_000), {
    ...
    const html = renderWatchDetailDocument(data!);
    expect(html).toContain("Watch digest: bets"); // the digest actually sent
    expect(html).toContain("woke on: runs: new activity in nightly runs");
    expect(html).toContain("ship the dispute queue");
    expect(html).toContain("Yesterday&#39;s bet.");
    expect(html).toContain("System prompt sent");
  });

  test("an oversized digest is capped with a pointer to the file on disk", async () => {
    await writeFile(join(paths.watchesDir, "bets.md"), BETS_FILE);
    await logCall("bets", ANCHOR, { userContent: "x".repeat(60_000) });

    const html = renderWatchDetailDocument((await collectWatchDetailPage(paths, "bets"))!);
