...
- **Procedural / normative (taste)** → **NEW judgment layer**, structured as:
  - A **taste unit** = `{reasoning, tier (hard|deterministic|soft-fuzzy), scope (always|glob|agent-requested), category (ideas|design|implementation|test-eval|communication|process — see §4c), evidence[] (immutable), canonical_example {bad,good}, recurrence, confidence, status, last_fired, decay}`.
...
- **`TEST_EVAL`** — *verification through to operations.* Testing strategy and eval design (what counts as adequate coverage, what "good verification" looks like, how to judge whether a result is actually good — directly serves the §2 "verification is the bottleneck" thesis), **plus deployment and operational soundness**: runbooks, pushing to staging, release mechanics, "did it actually ship and operate correctly." The unifying thread is *does it actually work, end to end.*
...
- **`PROCESS`** — meta-taste about *how the work is conducted*: workflow and methodology, documentation practice (what / when / how we document), planning structure, git and branch hygiene, when-to-ask-vs-proceed, security posture, tool discipline — the conventions refined through back-and-forth across many sessions. This is the **retrieved long tail** of working-practice taste; the curated, load-bearing operational rules still get promoted *upward* into the always-on identity / taste-principle files (§4f), and `PROCESS` is where the situational rest lives. (Boundary: operational *correctness* — runbooks executing, staging green — is `TEST_EVAL`; `PROCESS` is about working *method*, not shipping verification. And as with `IDEAS`/`COMMUNICATION`, the *substance* of process docs is `PROCESS` while how *well* they read is `COMMUNICATION`.)
...
- **Concrete shape:** a parallel taste store next to the fact canon (not inside it), sharded one file per category, with the deterministic tier compiled out to a checks dir:
  ...
  ├── ideas.md  design.md  implementation.md  test-eval.md  communication.md  process.md   # fuzzy units
